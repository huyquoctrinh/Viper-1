...
class RowParallelLinearFFN(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim):
        ...
        self.network = nn.Sequential(
            RowParallelLinear(input_dim, hidden_dim, bias=True, process_group=None, sequence_parallel=True),
            nn.ReLU(),
            RowParallelLinear(hidden_dim, output_dim, bias=True, process_group=None, sequence_parallel=True),
        )
...
class RowParallelLinearSwitchMixtureOfExperts(nn.Module):
    def __init__(
        ...
        self.router = RowParallelLinear(
            input_dim, num_experts, bias=True, process_group=None, sequence_parallel=True
        )

        # Experts: a list of FeedForward networks
        self.experts = nn.ModuleList(
            [
                RowParallelLinearFFN(input_dim, hidden_dim, expert_output_dim)
                for _ in range(num_experts)
            ]
        )
