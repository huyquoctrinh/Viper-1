...
import math
import os
from moe_lm.mamba_block import MambaBlock, MambaDecoder
from moe_lm.mamba_config import MambaConfig
from moe_lm.hf_utils import *
import os, json
...
class MambaModel(nn.Module):
    ...
    def forward(
        ...
        if input_ids is not None and input_embeddings is not None:
            raise ValueError("input_ids and input_embeddings should not be appeared as same time")

        if decoder_input is not None:
            pass
        elif input_embeddings is not None:
            decoder_input = input_embeddings
        elif self.pre_process and input_ids is not None:
    ...
    def save_pretrained(self, save_directory):
        ...
        serializable_config = {k: v for k, v in self.config.__dict__.items() if is_json_serializable(v)}
