...
from moe_lm.mamba_config import MambaConfig
...
from moe_lm.ops.selective_scan_interface import selective_scan_fn, mamba_inner_fn, SELECTIVE_SCAN_CUDA_IMPORT_FAILED

if SELECTIVE_SCAN_CUDA_IMPORT_FAILED:
    from moe_lm.ops.selective_scan_interface import selective_scan_ref
    selective_scan_fn  = selective_scan_ref
    mamba_inner_fn = None
    

try:
    from moe_lm.ops.triton.selective_state_update import selective_state_update
except ImportError:
    selective_state_update = None

try:
    from moe_lm.ops.triton.layernorm import RMSNorm, layer_norm_fn, rms_norm_fn
except ImportError:
...
class MambaLayer(nn.Module):
    def __init__(
        ...
        A = repeat(
            torch.arange(1, self.d_state + 1, dtype=torch.float32, device="cuda"),
            "n -> d n",
            d=self.d_inner,
        ).contiguous()
